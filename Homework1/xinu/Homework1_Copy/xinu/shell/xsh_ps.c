...
shellcmd xsh_ps(int nargs, char *args[])
{
	...
	printf("%3s %-16s %5s %4s %4s %10s %-10s %10s  %26s\n",
		   "Pid", "Name", "State", "Prio", "Ppid", "Stack Base",
		   "Stack Ptr", "Stack Size", "Time elasped since in ms");
	...
	for (i = 0; i < NPROC; i++) {
		...
		eTime = ( clktime*1000 + ctr1000 ) -(( prptr->prtimesec*1000) + prptr->prtimemillis);
		...
		printf("%3d %-16s %s %4d %4d 0x%08X 0x%08X %8d \t %d\n",
			...
			prptr->prstkptr, prptr->prstklen, eTime);
	}
	return 0;
}
